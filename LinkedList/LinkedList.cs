    ...
    public partial class LinkedList : Form
    {
       ...
       */
        ...
        private void Button2__Click(object sender, EventArgs e)
        {
            ...
            while(linkedListNode!= null)
            {
                LinkedListNode<object> copiedNode = new LinkedListNode<object>(0);
                copiedNode.Value = linkedListNode.Value;
                reverseLinkedList.AddLast(copiedNode);
                linkedListNode = linkedListNode.Previous;
            }
        ...
        private void Button4__Click(object sender, EventArgs e)
        {
            ...
            // 2. Your code here
            string sad = "Because I'm sad Clap along if you feel like a room without a roof Because I'm sad Clap along if you feel like sadness is the truth sad";
            s = sad.Split(' ');
            ...
            while (linkedListNode != null)
            {
                if((string)linkedListNode.Value == "sad")
                {
                    linkedListNode.Value = (object)"happy";
                }
                if((string)linkedListNode.Value == "sadness")
                {
                    linkedListNode.Value = (object)"happiness";
                }
                linkedListNode = linkedListNode.Next;
            }
        ...
        private void Button5__Click(object sender, EventArgs e)
        {
            ...
            // 2. Your code here
            string spain = "The Spain in rain falls plain on the mainly";
            s = spain.Split(' ');
            linkedList = new LinkedList<object>(s);
            ...
            LinkedListNode<object> currentTwo = linkedListNode2.Previous;
            LinkedListNode<object> currentOne = linkedListNode1.Previous;
            linkedList.Remove(linkedListNode1);
            linkedList.Remove(linkedListNode2);
            linkedList.AddAfter(currentTwo, linkedListNode1);
            linkedList.AddAfter(currentOne, linkedListNode2);

            linkedListNode1 = linkedList.Find("mainly");
            linkedListNode2 = linkedList.Find("plain");
            currentOne = linkedListNode1.Previous;
            currentTwo = linkedListNode2.Previous;
            linkedList.Remove(linkedListNode1);
            linkedList.Remove(linkedListNode2);
            linkedList.AddAfter(currentOne, linkedListNode2);
            linkedList.AddAfter(currentTwo, linkedListNode1);
        ...
        private void Button6__Click(object sender, EventArgs e)
        {
            ...
            // Your code here
            LinkedListNode<object> node1 = anagram.Find("O");
            LinkedListNode<object> node2 = anagram.Find("I");
            LinkedListNode<object> current = node1.Previous;
            anagram.Remove(node2);
            anagram.AddAfter(current, node2);
            current = anagram.FindLast("O");
            anagram.Remove(node1);
            anagram.AddAfter(current, node1);
            //anagram reads DIRMTOORY
            node1 = anagram.Find("M");
            node2 = anagram.Find("Y");
            current = node1.Next;
            anagram.Remove(node1);
            anagram.AddLast(node1);
            anagram.Remove(node2);
            anagram.AddAfter(current, node2);
            //anagram reads DIRTYOORM
            node1 = anagram.FindLast("R");
            current = anagram.Find("O");
            anagram.Remove(node1);
            anagram.AddBefore(current, node1);

            //not a very elegant solution. but a solution. ideally I'd make a smaller method just to swap two nodes, the method of which takes the param of a ref linkedlist
