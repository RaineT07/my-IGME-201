...
namespace PeopleApp
{
    class Program
    {
        ...
        {
            //R: create our new course list
            Courses myCourses = new Courses();

            //R: create a reference variable to access course object
                    ...
                    case "list":
                        ...
                        foreach (KeyValuePair<string, Person> thisEntry in people.sortedList)
                        {
                            ...
                            if (thisPerson.GetType() == typeof(Student))
                            {
                                ...
                                int courseIndex = student.courseCodes.Count;
   
                                for (int x = 0; x <courseIndex; x++)
                                {
                                    myCourse = myCourses.sortedList[$"IGME-{student.courseCodes[x]}"];
                                    int daysCounter = myCourse.schedule.daysOfWeek.Count;
                                    Console.Write($"\t IGME-{student.courseCodes[x]} - {myCourse.description} ");
                                    for (int j = 0; j < daysCounter; j++)
                                    {
                                        Console.Write($"{myCourse.schedule.daysOfWeek[j]} ");
                                    }
                                    Console.Write($"{myCourse.schedule.startTime:hh:mmtt} - ");
                                    Console.WriteLine($"{myCourse.schedule.endTime:hh:mmtt}");
                                }
        ...
        public static void EditPerson(ref Person thisPerson)
        {
            ...
            if(thisPerson.GetType() == typeof(Student))
            {
                //explicitly casts the person into a student
                Student thisStudent = (Student)thisPerson;

                do
                {
                    //prompt for course code, store it as a string for now
                    Console.Write($"course code=> ");
                    //store user input as string and initialize validCourseCode
                    string thisCourseCode = Console.ReadLine();
                    int validCourseCode = 0;
                    if (thisCourseCode.Length > 0)
                    {
                        //check if user input is an int between 200 and 299
                        if (int.TryParse(thisCourseCode, out validCourseCode))
                        {
                            //if conditions are properly met, store original string user input into student courseCodes
                            if(validCourseCode>199 && validCourseCode<300)
                            {
                                thisStudent.courseCodes.Add(thisCourseCode);
                            }
                            else
                            {
